...
class OMODMakerWidget(BaseWidget):
    ...
    def create_plugin(self):
        ...
        if not name:
            QtWidgets.QMessageBox.warning(self, 'Validation', 'Please enter a plugin name.')
            return
        if not out_dir:
            QtWidgets.QMessageBox.warning(self, 'Validation', 'Please choose an output folder (Settings are stored in config.json).')
            return
        # persist config
        if getattr(self, 'cfg', None) is not None:
            try:
        ...
        try:
            ...
            QtWidgets.QMessageBox.critical(self, 'Error', f'Failed to create plugin:\n{e}')
            return
